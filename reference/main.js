...
function main(){
	...
	initializeCamera();
	let ctxA=CANVAS_A.getContext("2d");
	let ctxB=CANVAS_B.getContext("2d");
	let ctxPiano=CANVAS_PIANO.getContext("2d");
	let ctxBg=CANVAS_BG.getContext("2d");
...
function getDifference(aImgData,bImgData){
	for(let y=0;y<aImgData.height;y++){
		for(let x=0;x<aImgData.width;x++){
			let aPx=getPixelValue(aImgData.data,x,y);
			let bPx=getPixelValue(bImgData.data,x,y);
...
function getXYSize(color){
	let x=color[0]; //red is index 0
	let y=color[2]; //blue is index 2
	
	x+=zOffset[0]*(color[1]/255); // green is index 1
	y+=zOffset[1]*(color[1]/255); // green is index 1
	
	let size=2+5*(color[1]/255);
