...
function drawScene() {
  //this function handles all drawing
...
function drawHouse(ctx, location, scale, properties) {
  ...
  for (var i = 1; i <= properties.levels; i++) {
    ctx.beginPath();
  ...
  ctx.restore(); //door of house

  if (properties.door) {
    ctx.save();
    ctx.translate(location[0], location[1]);
    ctx.scale(scale * 0.5, scale * 0.5);
    ctx.lineWidth = 0.4; //why are we doing this again? the new translation of our walls from 0 to 0.1, and the weird random angles will be maintained otherwise
...
function drawBackground(ctx) {
  ...
  ctx.beginPath();
  ctx.fillStyle = 'green';
  ctx.rect(0, size * 0.5, size, 0.5 * size); //read this as x starts all the way to the left(x=0, height starts at 50% of totalheight, width is totalwidth, and height is half totalheight
